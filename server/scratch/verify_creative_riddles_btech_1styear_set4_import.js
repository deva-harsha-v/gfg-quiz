...
async function verifyImportBTechSet4() {
  try {
    console.log('--- DB VERIFICATION FOR CREATIVE RIDDLES B.TECH 1ST YEAR SET 4 ---');
    
    // 1. Verify target round
    const btechSet4Round = await QuizRound.findOne({
      where: {
    ...
    if (!btechSet4Round) {
      throw new Error('❌ Target round Creative Riddles - B.Tech 1st Year — SET 4 NOT found!');
    }

    console.log(`✅ Target Round Found: "${btechSet4Round.title}"`);
    console.log(`   ID: ${btechSet4Round.id}`);
    console.log(`   Category: ${btechSet4Round.category}`);
    console.log(`   Course: ${btechSet4Round.course}`);
    console.log(`   Year: ${btechSet4Round.year}`);
    console.log(`   Set Number: ${btechSet4Round.setNumber}`);
    console.log(`   Status: ${btechSet4Round.status} (UNCHANGED)`);

    // Check duplicate rounds for SET 4
    const btechSet4RoundDuplicates = await QuizRound.count({
      where: {
        category: 'Creative Riddles',
        course: 'B.Tech',
        year: '1st Year',
    ...
    console.log(`   Duplicate Rounds Count: ${btechSet4RoundDuplicates - 1}`);

    // 2. Verify questions in SET 4
    const questions = await Question.findAll({
      where: { roundId: btechSet4Round.id },
      order: [['questionOrder', 'ASC']]
    });

    console.log(`✅ Question Count in B.Tech 1st Year SET 4: ${questions.length}`);
    ...
    console.log(`TARGET:
Creative Riddles - B.Tech 1st Year — SET 4
