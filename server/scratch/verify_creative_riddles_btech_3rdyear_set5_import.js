...
async function verify() {
  try {
    console.log('=== MYSQL VERIFICATION FOR CREATIVE RIDDLES B.TECH 3RD YEAR SET 5 ===\n');
    ...
    if (duplicateOrders === 0 && duplicateQuestions === 0) {
      console.log(`[PASS] Zero duplicates found in SET 5.`);
    } else {
    ...
    if (matchingRounds.length === 1) {
      ...
      console.error(`[FAIL] Multiple rounds found for SET 5!`);
    }

    // 5. Verify all B.Tech 3rd Year sets
    console.log('\n--- Checking All B.Tech 3rd Year Sets ---');
    const btech3rdYrSets = await QuizRound.findAll({
      where: {
        category: 'Creative Riddles',
        course: 'B.Tech',
        year: '3rd Year'
