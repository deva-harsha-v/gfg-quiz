...
const ROUND_2_ID = '252d32d6-3796-4827-94ac-1b67fcba131a';
...
async function testRound2() {
  try {
    console.log('====================================================');
    console.log('🧪 ROUND 2 END-TO-END VERIFICATION TEST');
    console.log('====================================================');
    ...
    // 2. Fetch Round 2 Questions via Admin API
    const adminQData = await get(`${API}/rounds/${ROUND_2_ID}/questions`, adminToken);
    console.log(`✅ Admin retrieved ${adminQData.questions.length} questions for Round 2`);
    console.log(`   Sample Admin view (shows correctOption): Q1 correct = ${adminQData.questions[0].correctOption}`);

    // 3. Activate Round 2
    await post(`${API}/rounds/${ROUND_2_ID}/activate`, {}, adminToken);
    console.log('✅ Round 2 activated');

    // 4. Participant registration & login
    const regData = await post(`${API}/auth/participant/register`, {
      name: 'Round 2 Tester',
      rollNumber: `R2TEST${Date.now().toString().slice(-4)}`,
      department: 'CSE',
    ...
    // 5. Start Round 2 attempt
    const startData = await post(`${API}/quiz/rounds/${ROUND_2_ID}/start`, {}, partToken);
    ...
    // 6. Save answer for Q1 (Correct: B) and Q2 (Correct: B)
    const q1 = questions.find(q => q.questionOrder === 1);
    const q2 = questions.find(q => q.questionOrder === 2);

    await put(`${API}/quiz/attempts/${attemptId}/questions/${q1.id}/answer`, { selectedOption: 'B' }, partToken);
    await put(`${API}/quiz/attempts/${attemptId}/questions/${q2.id}/answer`, { selectedOption: 'B' }, partToken);
    console.log('✅ Answers saved for Q1 (B) and Q2 (B)');
    ...
    console.log('====================================================');
    console.log('🎉 ROUND 2 END-TO-END TEST PASSED PERFECTLY!');
    console.log('====================================================');
  } catch (err) {
    console.error('❌ Round 2 E2E test failed:', err.message);
    process.exit(1);
  } finally {
    process.exit(0);
  }
