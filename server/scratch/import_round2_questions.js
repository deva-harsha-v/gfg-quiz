...
async function run() {
  ...
  try {
    ...
    // 1. Find Round 2
    let round2 = await QuizRound.findOne({ where: { roundNumber: 2 }, transaction });
    if (!round2) {
      console.log('Round 2 not found by roundNumber=2, searching by title containing "Round 2"...');
      round2 = await QuizRound.findOne({ where: { title: { [sequelize.Op.like]: '%Round 2%' } }, transaction });
    }

    if (!round2) {
      throw new Error('Round 2 does not exist in database!');
    }

    console.log(`Found Round 2: ID=${round2.id}, Title="${round2.title}", Status=${round2.status}`);

    // 2. Check existing questions count in Round 2
    const existingCount = await Question.count({ where: { roundId: round2.id }, transaction });
    console.log(`Existing questions count in Round 2: ${existingCount}`);
    ...
    for (const qData of questionsData) {
      ...
      if (existingQ) {
        duplicateCount++;
        // Update existing question data to match exact set
    ...
    const finalCount = await Question.count({ where: { roundId: round2.id } });

    console.log('====================================================');
    console.log('🎉 ROUND 2 QUESTIONS IMPORT COMPLETED SUCCESSFULLY');
    console.log('====================================================');
    console.log(`Round 2 ID: ${round2.id}`);
    console.log(`Questions inserted: ${insertedCount}`);
    console.log(`Duplicates found/updated: ${duplicateCount}`);
    console.log(`Final Question Count in Round 2: ${finalCount}`);

  } catch (error) {
    await transaction.rollback();
    console.error('❌ Failed to import Round 2 questions:', error);
    process.exit(1);
  } finally {
    process.exit(0);
  }
