...
async function importSet1() {
  const t = await sequelize.transaction();
  try {
    await sequelize.authenticate();
    console.log('Connected to database.');

    // 1. Fetch Set 1 record (roundNumber = 1)
    const [[set1]] = await sequelize.query(
      `SELECT * FROM quiz_rounds WHERE roundNumber = 1`,
      { transaction: t }
    );

    if (!set1) {
      throw new Error(`Set 1 record (roundNumber = 1) not found!`);
    }

    // Ensure title is 'Set 1'
    await sequelize.query(
      `UPDATE quiz_rounds SET title = 'Set 1' WHERE id = :id`,
      { replacements: { id: set1.id }, transaction: t }
    );

    console.log(`Found Set 1 record: ID = ${set1.id}, Title = "Set 1", Status = ${set1.status}`);
    ...
    console.log(`Old questions count for Set 1: ${oldCount}`);

    // 3. Delete old questions belonging ONLY to Set 1
    if (oldCount > 0) {
      await sequelize.query(
        `DELETE FROM questions WHERE roundId = :id`,
        { replacements: { id: set1.id }, transaction: t }
      );
      console.log(`Deleted ${oldCount} old questions from Set 1.`);
    }

    // 4. Insert 30 questions into Set 1
    ...
    console.log(`Inserted ${questionsData.length} questions into Set 1.`);

    await t.commit();
    console.log('Set 1 import transaction committed successfully!');
  } catch (err) {
    await t.rollback();
