...
async function verifyImportBTechSet1() {
  try {
    console.log('--- DB VERIFICATION FOR CREATIVE RIDDLES B.TECH 1ST YEAR SET 1 ---');
    
    // 1. Verify target round
    const btechSet1Round = await QuizRound.findOne({
      where: {
    ...
    if (!btechSet1Round) {
      throw new Error('❌ Target round Creative Riddles - B.Tech 1st Year — SET 1 NOT found!');
    }

    console.log(`✅ Target Round Found: "${btechSet1Round.title}"`);
    console.log(`   ID: ${btechSet1Round.id}`);
    console.log(`   Category: ${btechSet1Round.category}`);
    console.log(`   Course: ${btechSet1Round.course}`);
    console.log(`   Year: ${btechSet1Round.year}`);
    console.log(`   Set Number: ${btechSet1Round.setNumber}`);
    console.log(`   Status: ${btechSet1Round.status} (UNCHANGED)`);

    // Check duplicate rounds for SET 1
    const btechSet1RoundDuplicates = await QuizRound.count({
      where: {
        category: 'Creative Riddles',
        course: 'B.Tech',
        year: '1st Year',
    ...
    console.log(`   Duplicate Rounds Count: ${btechSet1RoundDuplicates - 1}`);

    // 2. Verify questions in SET 1
    const questions = await Question.findAll({
      where: { roundId: btechSet1Round.id },
      order: [['questionOrder', 'ASC']]
    });

    console.log(`✅ Question Count in B.Tech 1st Year SET 1: ${questions.length}`);
    ...
    // 3. Verify Diploma Sets UNCHANGED
    ...
    console.log(`TARGET:
Creative Riddles - B.Tech 1st Year — SET 1
