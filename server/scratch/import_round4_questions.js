...
async function run() {
  ...
  try {
    ...
    // 1. Find Round 4
    let round4 = await QuizRound.findOne({ where: { roundNumber: 4 }, transaction });
    if (!round4) {
      console.log('Round 4 not found by roundNumber=4, searching by title containing "Round 4"...');
      round4 = await QuizRound.findOne({ where: { title: { [sequelize.Op.like]: '%Round 4%' } }, transaction });
    }

    if (!round4) {
      throw new Error('Round 4 does not exist in database!');
    }

    console.log(`Found Round 4: ID=${round4.id}, Title="${round4.title}", Status=${round4.status}`);

    // 2. Check existing questions count in Round 4
    const existingCount = await Question.count({ where: { roundId: round4.id }, transaction });
    console.log(`Existing questions count in Round 4: ${existingCount}`);
    ...
    const finalCount = await Question.count({ where: { roundId: round4.id } });

    console.log('====================================================');
    console.log('🎉 ROUND 4 QUESTIONS IMPORT COMPLETED SUCCESSFULLY');
    console.log('====================================================');
    console.log(`Round 4 ID: ${round4.id}`);
    console.log(`Questions inserted: ${insertedCount}`);
    console.log(`Duplicates found/updated: ${duplicateCount}`);
    console.log(`Final Question Count in Round 4: ${finalCount}`);

  } catch (error) {
    await transaction.rollback();
    console.error('❌ Failed to import Round 4 questions:', error);
    process.exit(1);
  } finally {
    process.exit(0);
  }
