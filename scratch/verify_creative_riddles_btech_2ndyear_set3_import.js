...
async function verifyImportBTech2ndYearSet3() {
  try {
    console.log('--- DB VERIFICATION FOR CREATIVE RIDDLES B.TECH 2ND YEAR SET 3 ---');
    
    // 1. Verify target round
    const btech2ndSet3Round = await QuizRound.findOne({
      where: {
    ...
    if (!btech2ndSet3Round) {
      throw new Error('❌ Target round Creative Riddles - B.Tech 2nd Year — SET 3 NOT found!');
    }

    console.log(`✅ Target Round Found: "${btech2ndSet3Round.title}"`);
    console.log(`   ID: ${btech2ndSet3Round.id}`);
    console.log(`   Category: ${btech2ndSet3Round.category}`);
    console.log(`   Course: ${btech2ndSet3Round.course}`);
    console.log(`   Year: ${btech2ndSet3Round.year}`);
    console.log(`   Set Number: ${btech2ndSet3Round.setNumber}`);
    console.log(`   Status: ${btech2ndSet3Round.status} (UNCHANGED)`);

    // Check duplicate rounds for SET 3
    const btech2ndSet3RoundDuplicates = await QuizRound.count({
      where: {
        category: 'Creative Riddles',
        course: 'B.Tech',
        year: '2nd Year',
    ...
    console.log(`   Duplicate Rounds Count: ${btech2ndSet3RoundDuplicates - 1}`);

    // 2. Verify questions in SET 3
    const questions = await Question.findAll({
      where: { roundId: btech2ndSet3Round.id },
      order: [['questionOrder', 'ASC']]
    });

    console.log(`✅ Question Count in B.Tech 2nd Year SET 3: ${questions.length}`);
    ...
    // 3. Verify B.Tech 2nd Year SET 1 & SET 2 UNCHANGED
    for (let setNum of [1, 2]) {
      const r = await QuizRound.findOne({ where: { category: 'Creative Riddles', course: 'B.Tech', year: '2nd Year', setNumber: setNum } });
      const qC = r ? await Question.count({ where: { roundId: r.id } }) : 0;
      console.log(`✅ Creative Riddles B.Tech 2nd Year SET ${setNum}: ${qC} Questions (UNCHANGED)`);
    }
    ...
    console.log(`TARGET:
Creative Riddles - B.Tech 2nd Year — SET 3
