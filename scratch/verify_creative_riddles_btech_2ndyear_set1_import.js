...
async function verifyImportBTech2ndYearSet1() {
  try {
    console.log('--- DB VERIFICATION FOR CREATIVE RIDDLES B.TECH 2ND YEAR SET 1 ---');
    
    // 1. Verify target round
    const btech2ndSet1Round = await QuizRound.findOne({
      where: {
    ...
    if (!btech2ndSet1Round) {
      throw new Error('❌ Target round Creative Riddles - B.Tech 2nd Year — SET 1 NOT found!');
    }

    console.log(`✅ Target Round Found: "${btech2ndSet1Round.title}"`);
    console.log(`   ID: ${btech2ndSet1Round.id}`);
    console.log(`   Category: ${btech2ndSet1Round.category}`);
    console.log(`   Course: ${btech2ndSet1Round.course}`);
    console.log(`   Year: ${btech2ndSet1Round.year}`);
    console.log(`   Set Number: ${btech2ndSet1Round.setNumber}`);
    console.log(`   Status: ${btech2ndSet1Round.status} (UNCHANGED)`);

    // Check duplicate rounds for SET 1
    const btech2ndSet1RoundDuplicates = await QuizRound.count({
      where: {
        category: 'Creative Riddles',
        course: 'B.Tech',
        year: '2nd Year',
    ...
    console.log(`   Duplicate Rounds Count: ${btech2ndSet1RoundDuplicates - 1}`);

    // 2. Verify questions in SET 1
    const questions = await Question.findAll({
      where: { roundId: btech2ndSet1Round.id },
      order: [['questionOrder', 'ASC']]
    });

    console.log(`✅ Question Count in B.Tech 2nd Year SET 1: ${questions.length}`);
    ...
    // 3. Verify B.Tech 1st Year Sets & Diploma Sets UNCHANGED
    ...
    console.log(`TARGET:
Creative Riddles - B.Tech 2nd Year — SET 1
