...
async function verifySet3Import() {
  try {
    await sequelize.authenticate();
    console.log('=== VERIFYING LOGICAL REASONING - B.TECH 2ND YEAR SET 3 ===\n');
    ...
    if (!set1 || !set2 || !set3) {
      console.error('❌ B.Tech 2nd Year SET 1, SET 2, or SET 3 round not found!');
      process.exit(1);
    }

    const set1Count = await Question.count({ where: { roundId: set1.id } });
    const set2Count = await Question.count({ where: { roundId: set2.id } });
    const set3Questions = await Question.findAll({
      where: { roundId: set3.id },
    ...
    console.log(`B.Tech 2nd Year SET 3 Question Count: ${set3Questions.length} (Expected: 30)`);
    ...
    if (set3Questions.length !== 30) {
      console.error(`❌ Expected 30 questions in B.Tech 2nd Year SET 3, found ${set3Questions.length}`);
      pass = false;
    }

    // Verify ordering Q1-Q30 & non-null correctOption
    for (let i = 1; i <= 30; i++) {
      const q = set3Questions.find(item => item.questionOrder === i);
      if (!q) {
